...
const returnDate = (date) => {
    ...
    return new Date(year, month, day, 00);
}
...
//   const day = dateObj.getUTCDate();
//   const year = dateObj.getUTCFullYear();
  const Day = returnDate(req.body.date);
  const nextDay = new Date(Day.setDate(Day.getDate()+1));
  await Event.find({ admin: req.body.admin, time: { $gte: Day, $lt: nextDay } })
    .then((response) => {
...
router.post("/AdminDeleteQueue", async (req, res) => {
  ...
  await Event.find({ time: { $gte: Day, $lt: nextDay } })
    .then(async (response) => {
      const s = [];
...
router.post("/AdminGetDayQueues", async (req, res) => {
  ...
  await Event.find({ admin: id, time: { $gte: Day, $lt: nextDay } })
    .then(async (response) => {
      const s = [];
