...
export default function Calendar() {
  ...
  async function schedulePushNotification(ddate, hhour) {
    ...
    const trigger = new Date(year, month - 1, day - 1, 16);
    console.log(trigger);
    await Notifications.scheduleNotificationAsync({
      content: {
        title: "נעמה מניקור 💅🏼",
        body: ` קיים תור אצלי מחר בשעה ${hhour}:00`,
      },
      trigger,
    });
  }
  ...
  useEffect(async () => {
    ...
    if (x.getDay() == 6 || x.getDay() == 5) {
      ...
      await client
        .post(
          "/events/getDayQueues",
          { date: selectedDate, admin: user.myAdmin },
          {
            headers: {
              "Content-Type": "application/json",
            },
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              />
                              <Text
                                style={{ textAlign: "right", fontSize: 19 }}
                              >
                                {" "}
                                {item.hour}:00
                              </Text>
